...
def get_opinions(term_page_url, authors, parse_pdf=True):
    ...
    for row in table:
        ...
        o = {
            "type": opinion_type,
            "created": datetime.datetime.now().strftime(df),
            "published": published.strftime(df),
            "name": name,
            "pdf_url": pdf_url,
            "reporter_id": reporter_id,
            "docket_num": row[1].text,
            "part_num": row[4].text,
            "author_id": authors.get(row[3].text, '?'),
            "urls": []
        }

        logging.info("found opinion: %s", o['pdf_url'])
...
def extract_urls(pdf_url):
    ...
    logging.info("parsing pdf: %s", pdf_url)
